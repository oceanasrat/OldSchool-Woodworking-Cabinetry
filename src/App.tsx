...
const palette = { cream:'#FAF7F2', charcoal:'#1F2937', copper:'#B87333', wood:'#8B5E34', border:'#E5E7EB' }

export default function App() {
  const { site, projects, testimonials } = useData()

  const [query, setQuery] = useState('')
  const [category, setCategory] =
    useState<'All'|'Kitchens'|'Bathrooms'|'Mudrooms'|'Laundry'|'Closets'|'Furniture'>('All')
  ...
  return (
    <div className="min-h-screen" style={{ background:palette.cream, color:palette.charcoal }}>
      ...
      <div className="sticky top-0 z-40 border-b"
           style={{ background:'rgba(250,247,242,.90)', backdropFilter:'blur(6px)', borderColor:palette.border }}>
        <div className="container-max py-3 flex items-center justify-between">
          <a href="#" className="flex items-center gap-3 font-semibold">
            ...
            />
            <span className="hidden md:inline display text-xl whitespace-nowrap"
          ...
          <div className="hidden sm:flex items-center gap-2">
            <a href="#contact" className="px-4 py-2 rounded-2xl border" style={{ borderColor:palette.border }}>
              Free Consultation
            </a>
            <button className="px-4 py-2 rounded-2xl text-white" style={{ background:palette.copper }}
                    onClick={() => setShowEstimator(true)}>
              Get a Quote
            </button>
          </div>
          <div className="sm:hidden">
            <a href="#contact" className="px-3 py-2 rounded-lg text-white" style={{ background:palette.copper }}>
              Free Consultation
            </a>
          </div>
        </div>
      ...
      {/* Hero */}
      <section className="container-max py-12 grid md:grid-cols-2 gap-10 items-center">
        <div>
          <div className="kicker">Fine cabinetry</div>
          <h1 className="display text-4xl md:text-5xl font-extrabold leading-tight" style={{ color:palette.wood }}>
            {site?.tagline || 'Bespoke cabinetry & furniture'}
          </h1>
          <p className="mt-4 max-w-xl opacity-80">
            Handcrafted pieces tailored to your space. We design, build, and install cabinetry, furniture,
            and built-ins with premium materials and traditional joinery.
          </p>
      ...
      {/* Parallax band */}
      <section className="h-56 md:h-72 bg-cover bg-center"
               style={{
      ...
      {/* Portfolio */}
      <section id="portfolio" className="container-max py-12">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            <div className="kicker">Work</div>
            <h2 className="section-title" style={{ color:palette.wood }}>Portfolio</h2>
            <p className="opacity-80">Interactive gallery with zoom-in. Filter by room type, material, or search by name.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <input className="px-3 py-2 rounded-xl border w-56" style={{ borderColor:palette.border }}
                   placeholder="Search projects…" value={query} onChange={e=>setQuery(e.target.value)} />
            <select className="px-3 py-2 rounded-xl border" style={{ borderColor:palette.border }}
                    value={category} onChange={e=>setCategory(e.target.value as any)}>
              {['All','Kitchens','Bathrooms','Mudrooms','Laundry','Closets','Furniture'].map(c => <option key={c}>{c}</option>)}
            </select>
            <select className="px-3 py-2 rounded-xl border" style={{ borderColor:palette.border }}
                    value={material} onChange={e=>setMaterial(e.target.value as any)}>
              {['All','Walnut','Oak','Ash','Maple'].map(m => <option key={m}>{m}</option>)}
            </select>
          </div>
        </div>
            ...
            <div key={p.id} className="rounded-2xl overflow-hidden card">
              ...
              <div className="p-5">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold display text-lg">{p.title}</h3>
                  <span className="px-2.5 py-1 text-xs rounded-full bg-neutral-100">{p.type}</span>
                </div>
                <p className="opacity-80 mt-1">{p.blurb}</p>
                <div className="flex flex-wrap gap-2 mt-3">
                  {p.metrics?.map((m:string, i:number) =>
                    <span key={i} className="px-2 py-1 text-xs rounded-full bg-neutral-50">{m}</span>
                  )}
                </div>
                <button className="w-full mt-4 px-4 py-2 rounded-xl border hover:opacity-90 flex items-center justify-center gap-2"
      ...
      <section id="case-studies" className="border-y" style={{ borderColor:palette.border, background:'#ffffffb3' }}>
        <div className="container-max py-12">
          <div className="kicker">Deep dives</div>
          <h2 className="section-title" style={{ color:palette.wood }}>Case Studies</h2>
          <div className="mt-6 grid lg:grid-cols-2 gap-6">
            {projects.slice(0,2).map((p:any) => (
              <div key={p.id} className="rounded-2xl p-5 card">
                <div className="flex items-center gap-2">
                  <h3 className="display text-lg font-semibold">{p.title}</h3>
                  <span className="px-2 py-0.5 text-xs rounded-full bg-neutral-50">{p.material||''}</span>
                </div>
                <p className="opacity-80 mt-1">{p.blurb}</p>
                ...
                <details className="mt-2"><summary className="cursor-pointer">Techniques</summary>
                  <p className="opacity-80 mt-2">Mortise & tenon joinery, edge-banding, pro spray-booth finishing.</p>
                </details>
              </div>
            ))}
          </div>
      ...
      <section id="about" className="container-max py-12">
        <div className="grid lg:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <h2 className="section-title" style={{ color:palette.wood }}>The Craftsman Story</h2>
            ...
            <div className="mt-5 grid sm:grid-cols-2 gap-3">
              <div className="p-4 card"><div className="font-semibold" style={{ color:palette.wood }}>Values</div><div className="opacity-80">Craft, transparency, kindness.</div></div>
              <div className="p-4 card"><div className="font-semibold" style={{ color:palette.wood }}>Guarantee</div><div className="opacity-80">2-year workmanship warranty.</div></div>
            </div>
          </div>
          <div className="p-5 card">
            <h3 className="font-semibold mb-3 display">Process Timeline</h3>
            <Timeline />
          </div>
        </div>
      </section>
      ...
      {/* Services */}
      <section id="services" className="container-max py-12">
        <div className="kicker">What we do</div>
        <h2 className="section-title" style={{ color:palette.wood }}>Services</h2>
        <div className="mt-6 grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={s.name} className="p-5 relative overflow-hidden card">
              <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full opacity-10" style={{ background:palette.copper }} />
              <div className="display font-semibold">{s.name}</div>
      ...
      <section id="testimonials" className="border-y" style={{ borderColor:palette.border, background:'#ffffffb3' }}>
        <div className="container-max py-12">
          <div className="kicker">Happy clients</div>
          <h2 className="section-title" style={{ color:palette.wood }}>Client Testimonials</h2>
          <div className="mt-6 grid md:grid-cols-3 gap-6">
      ...
      {/* Contact + Estimator hook */}
      <section id="contact" className="container-max py-12 grid lg:grid-cols-2 gap-10 items-start">
        <div>
          <div className="kicker">Start a project</div>
          <h2 className="section-title" style={{ color:palette.wood }}>Request a Free Consultation</h2>
          {estimateSummary && (
            <div className="p-4 mb-4 card">
              <div className="text-sm opacity-80 mb-2">AI Estimate summary (you can edit before sending):</div>
              <textarea className="w-full px-3 py-2 rounded-xl border min-h-[100px]" style={{ borderColor:palette.border }}
                        name="estimate" value={estimateSummary} onChange={(e)=>setEstimateSummary(e.target.value)} />
            </div>
          )}
        ...
        <div className="p-5 card text-sm opacity-90">
        ...
        <div className="container-max py-8 grid md:grid-cols-3 gap-6 text-sm">
          <div>
            <div className="font-semibold flex items-center gap-2">
              <Hammer className="w-4 h-4" /> {site?.name || 'Old School Woodworking and Cabinetry'}
            </div>
